...
void Snapshot::insertSystemInformation(){
...
void Snapshot::insertReadyDequeContent(){
...
void Snapshot::insertJobPoolContent(){
...
void Snapshot::insertDiskContent(){
	...
	for (int i = 0; i < mPtr->scanDiskDequesStatus.size(); ++i){
		...
		if (mPtr->scanDiskDequesStatus[i] == 1){
			...
			insertDiskContent2(itHigh, itHighEnd);
			...
			insertDiskContent2(itLow, itLowEnd);
		}
		else {
			itLow = mPtr->diskSets0[i].begin();
  			itLowEnd = mPtr->diskSets0[i].end();
...
template <typename T>
void Snapshot::insertDiskContent2(T& scanIt, const T& scanItEnd){
...
void Snapshot::insertMemoryInformation(){
...
void Snapshot::handleInputChoice(){
	...
	if (input != "r" && input != "p" && input != "c" && input != "d" &&
		...
		if (input == "d") {
			...
			insertReadyDequeContent();
			insertSystemInformation();
		}
		else if (input == "j"){
			insertJobPoolContent();
		}
		else if (input == "m"){
